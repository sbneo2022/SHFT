...
class Swap:
    ...
    def getExchangeIntersection(
        ...
        for item in bepswap:
            symbol_f = item.split(".")[1].split("-")[0] + "BNB"
            symbol_r = "BNB" + item.split(".")[1].split("-")[0]
            if symbol_f in exchange_reference.keys():
                return_me.append(item)
            elif symbol_r in exchange_reference.keys():
                return_me.append(item)
    ...
    def getReport(
        ...
        rune_assetDepth = pool_depths[BNB]["assetDepth"]
        rune_runeDepth = pool_depths[BNB]["runeDepth"]
        ...
        bnb_usd_price: Optional[Decimal] = None
        ...
        for key, value in exchange_prices.items():
            ...
            if key in BNB_USD and bnb_usd_price is None:
                ask_price, bid_price = value["ask_price"], value["bid_price"]
                bnb_usd_price = (ask_price + bid_price) / 2
        ...
        for capital_item in capitals:
            ...
            capital_in_bnb = capital_item / bnb_usd_price

            for name, item in pool_depths.items():
                payload = {
                    "base_usd_price": bnb_usd_price,
                    "capital_in_base": capital_in_bnb,
                ...
                if name == BNB:
                    exchange_product = exchange_dictionary["RUNEBNB"]
                    replace_ask_bid = False

                elif (product + "BNB") in exchange_dictionary:
                    exchange_product = exchange_dictionary[product + "BNB"]
                    replace_ask_bid = False

                elif ("BNB" + product) in exchange_dictionary:
                    exchange_product = exchange_dictionary["BNB" + product]
                ...
                if name == BNB:
                    ...
                    rune_we_swap = (
                        self._calc_swap_asset(assetDepth, runeDepth, capital_in_bnb)
                        - self.rune_commission
                    )
                    payload["case"]["1. Swap BNB to RUNE and get (RUNE)"] = rune_we_swap

                    bnb_we_get = rune_we_swap * (bid_price if market else ask_price)
                    payload["case"][
                        "2. We can buy using Market order (BNB)"
                    ] = bnb_we_get

                    roc = bnb_we_get / capital_in_bnb - 1
                    payload["roc"] = roc
                    ...
                    rune_we_get = capital_in_bnb / (ask_price if market else bid_price)
                    payload["case"][
                        "1. We can buy using Market order (RUNE)"
                    ] = rune_we_get

                    bnb_we_get = self._calc_swap_rune(
                        assetDepth, runeDepth, rune_we_get - self.rune_commission
                    )
                    payload["case"]["2. Swap RUNE to BNB and get (BNB)"] = rune_we_get
                    ...
                    rune_we_swap = (
                        self._calc_swap_asset(
                            rune_assetDepth, rune_runeDepth, capital_in_bnb
                        )
                        - self.rune_commission
                    )
                    payload["case"][
                        f"1. Doubleswap (1) BNB to {product} and get (RUNE)"
                    ] = rune_we_swap
                    ...
                    payload["case"][
                        f"2. Doubleswap (2) BNB to {product} and get ({product})"
                    ] = asset_we_get

                    bnb_we_get = asset_we_get * (bid_price if market else ask_price)

                    payload["case"][
                        "3. We can buy using Market order (BNB)"
                    ] = bnb_we_get
                    ...
                    # Case 2: Buy Asset on Exchange, then Swap to BNB on Bepswap:
                    asset_we_get = capital_in_bnb / (ask_price if market else bid_price)
                    payload["case"][
                        f"1. We can buy using Market order ({product})"
                    ] = asset_we_get
                    ...
                    payload["case"][
                        f"2. Doubleswap (1) {product} to BNB and get (RUNE)"
                    ] = rune_we_swap
                    ...
                    payload["case"][
                        f"3. Doubleswap (2) {product} to BNB and get (BNB)"
                    ] = bnb_we_get
