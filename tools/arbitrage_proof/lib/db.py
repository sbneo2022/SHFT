...
class Db:
    def __init__(self, config: dict, prefix: Optional[str] = None, dry_mode=False):
        ...
        if "method" not in self._config:
            logger.warning(
                f'No "method" field in config: no data will be written to database'
            )
            self._dry_mode = True
            return
        else:
        ...
        self._measurement = self._config["method"]
        self._measurement = (
            self._measurement
    ...
    def getPoint(
        self, time: datetime, fields: Optional[Union[str, list]] = None
    ) -> Optional[dict]:
        if self._dry_mode:
            return
        ...
        query = (
            f'select {fields} from "{self._measurement}" '
    ...
    def addPoint(self, fields: dict) -> Optional[str]:
        if self._dry_mode:
            return
        ...
        json_item = {
            ...
            "tags": {},
            "time": datetime.now(tz=timezone.utc).strftime(FORMAT),
            "fields": {},
        }
        ...
        for tag in TAG_FIELDS:
            if tag in self._config:
                json_item["tags"][tag] = self._config[tag]
