...
from tools.pancake.lib.venue.ape import Ape, NULL_ADDRESS
...
if __name__ == "__main__":
    ...
    good_products = {}
    for item, data in ape_tokens.items():
    ...
    ape = Ape({"ape": {"node": "https://bsc-dataseed.binance.org/"}})
    iterator = tqdm.tqdm(good_products)
    for symbol in iterator:
        iterator.set_description(f"Load decimals for {symbol:10}")
        contract = ape._w3.eth.contract(
            good_products[symbol]["address"], abi=ape._bsc_token_abi
        )
    ...
    for pair in iterator:
        ...
        try:
            ...
            target_pairs[pair]["pool"] = ape._ape_factory.functions.getPair(
                token0, token1
            ).call()
            if target_pairs[pair]["pool"] == NULL_ADDRESS:
                ...
                continue
            contract = ape._w3.eth.contract(
                target_pairs[pair]["pool"], abi=ape._ape_pair_abi
            )
