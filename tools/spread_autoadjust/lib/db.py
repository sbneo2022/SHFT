...
class Db:
    ...
    def getPoints(self, startDate, endDate, tags: dict=None, fields=None, where='WHERE ', **kwargs):
        if tags is not None:
            for key, value in tags.items():
                where += f'"{key}"=\'{value}\' AND '
        fields = [fields] if isinstance(fields, str) else fields
        fields = '*' if fields is None else ','.join([f'"{x}"' for x in fields])
        query = f'SELECT {fields} FROM "{self._measurement}" {where}' \
                f'TIME >= \'{startDate.strftime(FORMAT)}\' AND TIME < \'{endDate.strftime(FORMAT)}\' ' #GROUP BY time(1s) fill(previous)'
        reply = self._client.query(query, chunked=True, chunk_size=CHUNK_SIZE)
        df = pd.DataFrame(reply.get_points())
        df['time'] = df['time'].apply(lambda x: parse_datetime(x))
