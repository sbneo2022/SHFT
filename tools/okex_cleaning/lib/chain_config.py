...
def load_chain_config(default_config_path: Optional[str] = None) -> dict:
    ...
    parser.add_argument("-a", "--add", type=str, help="Additional YAML config or JSON item", action='append')
    args = parser.parse_args()

    config = get_config(default_config_path) if default_config_path is not None else dict()
